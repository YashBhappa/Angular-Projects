...
public class FlightRepositoryImpl implements FlightRepositoryCustom {
	...
	public List<Flight> searchFlight(FlightCriteria flightCriteria) {
		...
		if (flightCriteria.getDepartureLocation() != null) {
			predicates.add(
					cbuild.like(flight.get("departureLocation"), "%" + flightCriteria.getDepartureLocation() + "%"));
		}
		...
		if (flightCriteria.getDepartureDateMin() != null) {
			predicates.add(
					cbuild.greaterThanOrEqualTo(flight.get("departureDate"), flightCriteria.getDepartureDateMin()));
		}
		...
		if (flightCriteria.getDepartureTimeMin() != null) {
			predicates.add(
					cbuild.greaterThanOrEqualTo(flight.get("departureTime"), flightCriteria.getDepartureTimeMin()));
		}
		...
		if (flightCriteria.getConnectionDurationMax() != null) {
			predicates.add(cbuild.lessThanOrEqualTo(flight.get("connectionDuration"),
					flightCriteria.getConnectionDurationMax()));
		}

		if (flightCriteria.getFlightDurationMax() != null) {
			predicates
					.add(cbuild.lessThanOrEqualTo(flight.get("flightDuration"), flightCriteria.getFlightDurationMax()));
		}

		if (flightCriteria.getFlightDurationMin() != null) {
			predicates.add(
					cbuild.greaterThanOrEqualTo(flight.get("flightDuration"), flightCriteria.getFlightDurationMin()));
		}
	...
	public Long getNumberFlights(SynthesisCriteria synthesisCriteria) {
		...
		if (synthesisCriteria.getDepartureDateMax() != null) {
			predicates.add(
					cbuild.lessThanOrEqualTo(flight.get("departureDate"), synthesisCriteria.getDepartureDateMax()));
		}
		if (synthesisCriteria.getDepartureDateMin() != null) {
			predicates.add(
					cbuild.greaterThanOrEqualTo(flight.get("departureDate"), synthesisCriteria.getDepartureDateMin()));
		}
		cquery.where(predicates.toArray(new Predicate[0]));
